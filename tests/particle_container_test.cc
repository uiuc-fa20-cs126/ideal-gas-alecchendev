...
TEST_CASE("Constructor") {
  SECTION("Check all values are present") {
    ParticleContainer particle_container = ParticleContainer(50, 60, 30, vector<Particle>({
      Particle(1, 2, ci::Color(1, 0, 0), vec2(1, 2), vec2(2, 3))
    }));
    SECTION("Width") {
  ...
  SECTION("Check particles created") {
    ...
    SECTION("Multiple particle types") {
      ParticleContainer particle_container = ParticleContainer(50, 60, 30, vector<Particle>({
        Particle(1, 2, ci::Color(1, 0, 0), vec2(1, 2), vec2(2, 3)),
        Particle(2, 1, ci::Color(0, 1, 0), vec2(10, 5), vec2(3, 2))
      }));
      REQUIRE(particle_container.getParticles().size() == 30);
      REQUIRE(particle_container.getParticles()[0].radius == 1);
      REQUIRE(particle_container.getParticles()[0].mass == 2);
...
TEST_CASE("Check updating particle positions") {
  ...
  SECTION("Updating position with a velocity with nonzero length") {
    vec2 prev_pos = particle_container.getParticles()[0].position;
    particle_container.Update(1.0);
    REQUIRE(particle_container.getParticles()[0].position == prev_pos + particle_container.getParticles()[0].velocity);
  }
}
...
TEST_CASE("Check wall collisions") {
  ...
  SECTION("Left bound") {
    particle_container.AddParticle(Particle(1, 1, ci::Color(1, 0, 0), vec2(1, 20), vec2(-5, 0)));
    particle_container.Update(1);
    REQUIRE(particle_container.getParticles()[0].velocity.x == 5);
  }
  SECTION("Right bound") {
    particle_container.AddParticle(Particle(1, 1, ci::Color(1, 0, 0), vec2(49, 20), vec2(5, 0)));
    particle_container.Update(1);
    REQUIRE(particle_container.getParticles()[0].velocity.x == -5);
  }
  SECTION("Lower bound") {
    particle_container.AddParticle(Particle(1, 1, ci::Color(1, 0, 0), vec2(20, 1), vec2(0, 5)));
    particle_container.Update(1);
    REQUIRE(particle_container.getParticles()[0].velocity.y == 5);
  }
  SECTION("Upper bound") {
    particle_container.AddParticle(Particle(1, 1, ci::Color(1, 0, 0), vec2(20, 59), vec2(0, 5)));
    particle_container.Update(1);
    REQUIRE(particle_container.getParticles()[0].velocity.y == -5);
  }
}
...
TEST_CASE("Check particle collisions") {
  ...
  SECTION("Moving towards") {
    SECTION("Vertical and horizontal movement") {
      particle_container.AddParticle(Particle(1, 1, ci::Color(1, 0, 0), vec2(19, 19), vec2(1, 1)));
      particle_container.AddParticle(Particle(2, 2, ci::Color(1, 0, 0), vec2(22, 22), vec2(-1, -1)));
      particle_container.Update(1);
  ...
  SECTION("Within contact, but moving away") {
    particle_container.AddParticle(Particle(1, 1, ci::Color(1, 0, 0), vec2(20, 20), vec2(-1, -1)));
    particle_container.AddParticle(Particle(2, 2, ci::Color(1, 0, 0), vec2(22, 22), vec2(1, 1)));
    particle_container.Update(1);
